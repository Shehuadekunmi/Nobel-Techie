...
const Stepfour = () => {
  ...
  useEffect(() => {
    ...
    const step4Data = JSON.parse(localStorage.getItem("applicationStep4") || "{}");
    if (step4Data) {;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("organization", JSON.stringify(step2Data.organization || {}));
    ...
    <div className="evidence-container d-lg-flex">
      ...
      <div className="right-section">
        ...
        <div className="file-upload">
          {file1 ? (
            <>
              <a href={URL.createObjectURL(file1)} target="_blank" rel="noopener noreferrer">
                View Evidence
              </a>
              <button className="remove-btn" onClick={() => setFile1(null)}>Remove</button>
            </>
          ) : (
            <>
              <label className="label">Upload Supporting Evidence</label>
              <button type="button" className="upload-btn" onClick={() => document.getElementById("file1-upload").click()}>
                Upload
              </button>
              <input type="file" id="file1-upload" hidden onChange={(e) => handleFileChange(e, setFile1)} />
            </>
          )}
        </div>
        ...
        <div className="file-upload">
          {file2 ? (
            <>
              <a href={URL.createObjectURL(file2)} target="_blank" rel="noopener noreferrer">
                View CV
              </a>
              <button className="remove-btn" onClick={() => setFile2(null)}>Remove</button>
            </>
          ) : (
            <>
              <label className="label">Submit Your CV</label>
              <button type="button" className="upload-btn" onClick={() => document.getElementById("file2-upload").click()}>
                Upload
              </button>
              <input type="file" id="file2-upload" hidden onChange={(e) => handleFileChange(e, setFile2)} />
            </>
          )}
        </div>

        <div className="confirmation d-flex gap-2">
          <input type="checkbox" checked={confirmed} onChange={handleConfirmationChange} />
        ...
        <div className="buttons">
          ...
          <button className="next-btn" onClick={handleSubmit} disabled={isSubmitting || !file1 || !file2}>
